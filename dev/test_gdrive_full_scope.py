...
with patch.object(mod, "read_gdrive_client", return_value={}):
...
with patch.object(mod, "read_gdrive_client", return_value=CLIENT):
    cmd = mod._gdrive_authorize_command()
...
check("...and name the command that fixes it, in both languages",
      "/connectgdrive setupclient" in warn_en
      and "/connectgdrive setupclient" in warn_id)

# --- 4. the manual path actually passes both ------------------------------
src = SRC.read_text(encoding="utf-8")
manual = src.split("# step == \"await_gdrive_token\"")[1][:2500]
check("the manual path hands connect_gdrive_account the stored client",
      "read_gdrive_client()" in manual)
